...
export class Moodle {
	...
	private get moodleFramework(): MFReadyGlobalApi | undefined {
		if (!MF || !MF.createMoodle || !MF.getMoodle || !MF.ISMoodle?.new) {
			return undefined;
		}
	...
	constructor(props: MoodleProps) {
		...
		if (getActivatedMods().contains(MODS.MOODLE_FRAMEWORK)) {
			try {
				...
				print(
					`[BF][Warn] Moodle Framework mod detected but failed to load MF_ISMoodle. Falling back to HaloText for moodle "${name}".`
				);
				return;
			}
			...
			if (!this.moodleFramework) {
				print(
					`[BF][Warn] Moodle Framework mod detected but required APIs are missing. Falling back to HaloText for moodle "${name}".`
				);
				return;
			}
			...
			this.moodleFramework.ISMoodle.new(
				this.moodleFramework.ISMoodle,
				this.name,
				this.player
			);
		}
