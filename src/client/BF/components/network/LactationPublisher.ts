...
import {
	isBFPublishLactationStateResponse,
	BFPublishLactationStateRequest
...
export class LactationPublisher {
	...
	private pending?: {
		request: BFPublishLactationStateRequest;
	...
	private send(desired: LactationState, intent: LactationMutationIntent): void {
		...
		const request: BFPublishLactationStateRequest = {
	...
	private sendRequest(request: BFPublishLactationStateRequest): void {
		const desired = request.data.desired;
		print(
			`[BF][MP][Client] send PublishLactationStateRequest active=${desired.isActive} milk=${desired.milkAmount} expiration=${desired.expiration} multiplier=${desired.multiplier}`
		);
		sendClientCommand(
			getPlayer(),
			BF_NETWORK_MODULE,
	...
	public onServerCommand(module: string, command: string, args: unknown): void {
		if (
			...
			command !== BFNetworkCommand.PUBLISH_LACTATION_STATE_RESPONSE ||
			!isBFPublishLactationStateResponse(args) ||
		...
		print(
			`[BF][MP][Client] acknowledged PublishLactationStateResponse status=${args.status} active=${authoritative.isActive} milk=${authoritative.milkAmount} expiration=${authoritative.expiration} multiplier=${authoritative.multiplier}`
		);
		if (accepted) {
